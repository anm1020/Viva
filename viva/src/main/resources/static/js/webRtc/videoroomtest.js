...
$(document).ready(function() {
	// 야누스 라이브러리 초기화, debug:"all" -> 모든 디버그 메세지 콘솔에 출력
	// 디버깅 수준 설정: all, debug, warn, error
	// 초기화 완료되면 호출되는 콜백 함수
	Janus.init({debug: "all", callback: function() {
		// start 버튼 클릭시 한번만 실행
		$('#start').one('click', function() {

			// 버튼 중복 클릭 방지 : 비활성화 및 클릭 이벤트 제거
			$(this).attr('disabled', true).unbind('click');
			// 현재 브라우저가 webRTC 지원하는지 확인
			if(!Janus.isWebrtcSupported()) {
				// webRTC 지원하지 않을 경우 경고창
			...
			janus = new Janus(
				{
					...
					success: function() {	// 세션 생성 성공 시
						// videoroom 플러그인에 연결
						janus.attach(
							{
								plugin: "janus.plugin.videoroom",		// 사용할 플러그인
								opaqueId: opaqueId,						// 세션 식별용
								
								success: function(pluginHandle) {		// 플러그인 연결 성공시
									$('#details').remove();				// 설명 텍스트 제거
									sfutest = pluginHandle;				// 핸들 저장
									// 로그 출력
									Janus.log("Plugin attached! (" + sfutest.getPlugin() + ", id=" + sfutest.getId() + ")");
									Janus.log("  -- This is a publisher/manager");
									
									// 사용자 입력 UI 표시 -> 기존에 숨김처리 되어있었음
									$('#videojoin').removeClass('hide').show();
									$('#registernow').removeClass('hide').show();
									$('#register').click(registerUsername);			// "대화방 참여"" 버튼 클릭시 이벤트 연결
									$('#roomname').focus();						// 입력창 포커스
								...
								webrtcState: function(on) {
									...
									// 연결이 끊긴 경우, 이후 UI 처리 중단
									if(!on)	return;
									// 기존 "Publish" 버튼 제거	- 중복 방지
									$('#publish').remove();
									// 대역폭 조절 버튼영역 표시
								...
								// janus에서 받은 메시지 처리 (방 참여/나감/출판 등)
								onmessage: function(msg, jsep) {
									Janus.debug(" ::: Got a message (publisher) :::", msg);
									var event = msg["videoroom"];		// videoroom 관련 이벤트
									Janus.debug("Event: " + event);
									if(event) {
										// 방 참가 완료 이벤트
										if(event === "joined") {
											// Publisher/manager created, negotiate WebRTC and attach to existing feeds, if any
											myid = msg["id"];				// 내 janus ID
											mypvtid = msg["private_id"];	// 프라이빗 ID
											Janus.log("Successfully joined room " + msg["room"] + " with ID " + myid);
											
											// subscriber-only 모드이면 영상만 받기
											if(subscriber_mode) {
												$('#videojoin').hide();
												$('#videos').removeClass('hide').show();
											} else {
												publishOwnFeed(true);	// 직접 영상 송출 시작
											}
											// 기존 참가자들 목록이 있다면 원격 피드로 연결
											if(msg["publishers"]) {
